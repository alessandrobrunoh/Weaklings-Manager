...
const ALBION_ITEM_DISPLAY_NAMES: Readonly<Record<string, string>> = {
  ...
  MEAL_PIE: 'Pork Pie',
  MEAL_PIE_FISH: 'Frostpeak Deadeye Pie',
  MEAL_OMELETTE: 'Pork Omelette',
  MEAL_OMELETTE_FISH: 'Dusthole Crab Omelette',
  MEAL_OMELETTE_AVALON: 'Avalonian Pork Omelette',
  MEAL_STEW: 'Beef Stew',
  MEAL_STEW_FISH: 'Deadwater Eel Stew',
  MEAL_STEW_AVALON: 'Avalonian Beef Stew',
  MEAL_SANDWICH: 'Beef Sandwich',
  MEAL_SANDWICH_FISH: 'Thunderfall Lurcher Sandwich',
  MEAL_SANDWICH_AVALON: 'Avalonian Beef Sandwich',
  MEAL_ROAST: 'Roast Pork',
  MEAL_ROAST_FISH: 'Roasted Puremist Snapper',
  POTION_HEAL: 'Healing Potion',
  POTION_ENERGY: 'Energy Potion',
  POTION_REVIVE: 'Gigantify Potion',
...
/**
 * Consumables keep one identifier across tiers but change name with the tier — `T4_MEAL_STEW` is a
 * Goat Stew while `T8_MEAL_STEW` is a Beef Stew. Only the tiers that differ from the family name in
 * {@link ALBION_ITEM_DISPLAY_NAMES} are listed; anything absent falls back to that family name.
 */
const ALBION_TIERED_ITEM_DISPLAY_NAMES: Readonly<Record<string, string>> = {
...
export function normalizeAlbionEquipmentName(identifier: string, fallback = identifier): string {
   const tiered = ALBION_TIERED_ITEM_DISPLAY_NAMES[identifier.trim().toUpperCase()];
...
function toOpenAlbionItem(fileName: string, tier: string): OpenAlbionItem {
  ...
  return {
    ...
    name:
      ALBION_TIERED_ITEM_DISPLAY_NAMES[identifier] ??
      ALBION_ITEM_DISPLAY_NAMES[fileName] ??
