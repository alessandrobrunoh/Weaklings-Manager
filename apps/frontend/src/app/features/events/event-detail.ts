  ...
  template: `
          ...
          @case ('roster') {
            @if (rosterSnapshotState() === 'ready' && rosterSnapshot(); as roster) {
              <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-[250px_1fr_310px] xl:grid-cols-[270px_1fr_340px] gap-4 items-start">

                  <!-- 1. LEFT SIDEBAR: PANCHINA & CONTROLLI -->
                  <aside class="space-y-3 sticky top-4 max-h-[calc(100vh-6rem)] overflow-y-auto scrollbar-thin">
                    <!-- Panchina Card -->
                    <div
                    ...
                    >
                      <!-- Bench Header -->
                      <div class="bg-[var(--color-surface-1)] p-3 border-b border-[var(--color-border)] space-y-2">
                      ...
                      @if (isDropTargetBench()) {
                        <div class="m-2 p-2.5 rounded-lg border-2 border-dashed border-amber-500 bg-amber-500/15 text-amber-400 text-xs font-bold text-center animate-pulse flex items-center justify-center gap-2">
                          <app-icon name="users" size="0.875rem" />
                            ...
                            <div class="text-[10px] text-[var(--color-text-secondary)] space-y-0.5">
                              ...
                              @if (member.secondary_build_name) {
                                <div class="flex items-center justify-between gap-1">
                                  ...
                                  <span class="text-[var(--color-text-secondary)] truncate max-w-[10rem]">
                    ...
                    <div class="card p-3 space-y-3">
                      <div class="flex items-center justify-between">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
                          Capacità Roster
                        </span>
                      ...
                      <div class="h-1.5 w-full bg-[var(--color-surface-2)] rounded-full overflow-hidden">
                        <div
                          class="h-full bg-emerald-500 rounded-full transition-all"
                      ...
                      <!-- Weapon Spec Filter -->
                      <div class="space-y-1">
                        <span class="text-[10px] text-[var(--color-text-secondary)] font-medium block">Filtro Spec:</span>
                        <select
                  ...
                  <div class="space-y-4 min-w-0">
                    ...
                    @if (rosterSwapSource(); as source) {
                      <div class="card p-3 border border-[var(--color-primary)] bg-[var(--color-surface-2)] flex items-center justify-between gap-3 shadow-md animate-pulse">
                        <div class="flex items-center gap-2 text-xs font-semibold text-white">
                          <app-icon name="refresh" size="1rem" class="text-[var(--color-primary)]" />
                          <span>Seleziona il posto di destinazione per <strong>{{ source.participant?.username }}</strong> ({{ source.build_name }})</span>
                        </div>
                        <button type="button" class="btn btn--outline btn--sm text-xs" (click)="cancelRosterCommandMode()">
                      ...
                      <div class="card p-3 border border-[var(--color-primary)] bg-[var(--color-surface-2)] flex items-center justify-between gap-3 shadow-md animate-pulse">
                        <div class="flex items-center gap-2 text-xs font-semibold text-white">
                          ...
                          <span>Seleziona un membro dalla panchina per Party {{ target.party_number }}, pos. #{{ target.position }} ({{ target.build_name }})</span>
                        </div>
                        <button type="button" class="btn btn--outline btn--sm text-xs" (click)="cancelRosterCommandMode()">
                          {{ t('events.detail.cancel_swap') }}
                        </button>
                    ...
                    <!-- Parties Grid -->
                    <div class="grid gap-4" [class.md:grid-cols-2]="rosterParties().length > 1">
                      @for (party of rosterParties(); track party.partyNumber) {
                        <section class="card p-0 overflow-hidden shadow-sm border border-[var(--color-border)]">
                          <!-- Party Header -->
                          <header class="flex items-center justify-between gap-2 border-b border-[var(--color-border)] bg-[var(--color-surface-1)] px-3.5 py-2.5">
                            ...
                            <div class="flex items-center gap-2">
                              <div class="h-1.5 w-14 bg-[var(--color-surface-2)] rounded-full overflow-hidden">
                                <div
                                  class="h-full bg-emerald-500 rounded-full transition-all"
                                  [style.width.%]="(rosterPartyFilledSeats(party) / party.seats.length) * 100"
                                ></div>
                              </div>
                              <span class="font-mono text-xs font-bold text-white">
                          ...
                          <ol class="divide-y divide-[var(--color-border)]" [attr.aria-label]="rosterPartyName(party)">
                            @for (seat of party.seats; track seat.party_number + ':' + seat.position) {
                              <li
                                ...
                                [class.bg-[var(--color-surface-2)]]="rosterSwapSource()?.key === rosterSeatKey(seat) || activeInspectSeat()?.key === seat.key"
                                [class.ring-1]="activeInspectSeat()?.key === seat.key"
                                [class.ring-[var(--color-primary)]]="activeInspectSeat()?.key === seat.key"
                                ...
                                <div class="flex items-center gap-2.5 min-w-0 flex-1">
                                  ...
                                  >
                                    @if (seatWeaponIconUrl(seat); as icon) {
                                      <img [src]="icon" [alt]="seat.build_name" class="h-full w-full object-contain pointer-events-none" loading="lazy" />
                                    } @else {
                                  ...
                                  <div class="min-w-0 flex-1">
                                    <div class="flex items-center gap-1.5 flex-wrap">
                                      <span class="text-[9px] uppercase font-bold px-1.5 py-0.2 rounded" [class]="roleChip(seat.role)">
                                        {{ rosterSeatRoleLabel(seat) }}
                                      </span>
                                      <span class="text-xs font-bold text-white truncate max-w-[11rem]">
                                        {{ seat.build_name }}
                                      </span>
                                      ...
                                      @if (seat.participant; as participant) {
                                        <div class="flex items-center gap-1.5 min-w-0">
                                          ...
                                          <span
                                            ...
                                            [class.text-[var(--color-primary)]]="participant.user_id === currentParticipant()?.user_id"
                                            [class.font-bold]="participant.user_id === currentParticipant()?.user_id"
                                            [class.text-white]="participant.user_id !== currentParticipant()?.user_id"
                                          >
                                          ...
                                          @if (selectedSpecializationKey()) {
                                            <span
                                              ...
                                              [class.bg-emerald-500/10]="participantSpecLevel(participant) >= 100"
                                              [class.text-emerald-400]="participantSpecLevel(participant) >= 100"
                                              [class.bg-amber-500/10]="participantSpecLevel(participant) > 0 && participantSpecLevel(participant) < 100"
                                              [class.text-amber-400]="participantSpecLevel(participant) > 0 && participantSpecLevel(participant) < 100"
                                              [class.text-[var(--color-text-disabled)]]="participantSpecLevel(participant) === 0"
                                            >
                                        ...
                                        <span class="text-[11px] italic text-[var(--color-text-disabled)]">
                                          Posto libero
                                        </span>
                                      }
                                    </div>
                        ...
                        <p class="text-center py-8 text-xs text-[var(--color-text-secondary)]">Nessun party configurato.</p>
                      }
                    </div>
                  </div>

                  <!-- 3. RIGHT SIDEBAR: SIDEBAR CON EQUIP -->
                  <aside class="card p-3.5 space-y-3.5 sticky top-4 max-h-[calc(100vh-6rem)] overflow-y-auto scrollbar-thin">
                    @if (activeInspectSeat(); as seat) {
                      ...
                      <div class="space-y-2 border-b border-[var(--color-border)] pb-3">
                        <div class="flex items-center justify-between gap-2">
                          ...
                          @if (selectedInspectSeat() && ownRosterSeat() && selectedInspectSeat()?.key !== ownRosterSeat()?.key) {
                            <button
                              type="button"
                        ...
                        <div class="flex items-center justify-between gap-2">
                          <div class="min-w-0">
                            <h3 class="text-sm font-bold text-white truncate">{{ seat.build_name }}</h3>
                            <p class="text-[11px] text-[var(--color-text-secondary)] font-mono">
                              Party {{ seat.party_number }} · Pos. #{{ seat.position }}
                            </p>
                          </div>
                          <span class="chip font-semibold text-[10px]" [class]="roleChip(seat.role)">
                            {{ rosterSeatRoleLabel(seat) }}
                          </span>
                        </div>
                        ...
                        @if (seat.participant; as p) {
                          <div class="flex items-center gap-1.5 text-xs text-[var(--color-text-secondary)] bg-[var(--color-surface-2)] px-2 py-1 rounded-lg border border-[var(--color-border)]">
                            <app-avatar [username]="p.username" size="xs" />
                            <span class="truncate font-medium text-white">{{ p.username }}</span>
                          </div>
                        } @else {
                          <div class="text-xs text-[var(--color-text-tertiary)] italic bg-[var(--color-surface-2)] px-2 py-1 rounded-lg border border-[var(--color-border)]">
                            Posto non occupato
                          </div>
                        }
                        ...
                        @if (rosterSeatBuildItems(seat).some(i => i.loadout === 'swap')) {
                          <div class="flex items-center gap-1 p-0.5 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-2)]">
                            <button
                              ...
                              [class.text-[var(--color-text-secondary)]]="inspectLoadout() !== 'main'"
                            ...
                            <button
                              ...
                              [class.text-[var(--color-text-secondary)]]="inspectLoadout() !== 'swap'"
                      ...
                      <div>
                        <p class="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-secondary)] mb-2">
                      ...
                      <div class="space-y-2 pt-2 border-t border-[var(--color-border)]">
                        <div class="flex items-center justify-between">
                          <p class="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
                            Abilità scelte
                          </p>
                          <span class="text-[10px] text-[var(--color-text-tertiary)]">Spell configurate</span>
                        </div>
                        ...
                        @if (rows.length > 0) {
                          <div class="space-y-2">
                            @for (row of rows; track row.slot) {
                              <div class="p-2.5 bg-[var(--color-surface-2)] rounded-lg border border-[var(--color-border)] space-y-1.5">
                                ...
                                <div class="grid gap-1">
                                  @for (slot of row.slots; track slot.label + slot.index) {
                                    ...
                                    @if (chosen) {
                                      <div class="flex items-center gap-2 rounded bg-[var(--color-surface)] px-2 py-1 border border-white/5">
                                        <span class="font-mono text-[10px] font-bold px-1 py-0.2 rounded bg-white/10 text-white min-w-5 text-center">
                                          {{ slot.label }}
                                        </span>
                                        <img
                                          ...
                                          loading="lazy"
                                        />
                                        <span class="text-xs text-white font-medium truncate">{{ chosen.name }}</span>
                                      </div>
                                    }
                                  }
                                </div>
    ...
    @if (inspectDialogSeat(); as seat) {
      <app-dialog
        ...
        [subtitle]="'Party ' + seat.party_number + ' · Pos. #' + seat.position + ' · ' + rosterSeatRoleLabel(seat) + (seat.participant ? ' (' + seat.participant.username + ')' : ' (Libero)')"
        size="lg"
        (closed)="inspectDialogSeat.set(null)"
      >
        <div class="space-y-4">
          <!-- Role & Player info banner -->
          <div class="flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)]">
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs font-mono text-[var(--color-text-secondary)]">#{{ seat.position }}</span>
                </div>
                @if (seat.participant; as p) {
                  <div class="flex items-center gap-1.5 mt-1 text-xs text-[var(--color-text-secondary)]">
                    <app-avatar [username]="p.username" size="xs" />
                    <span>Assegnato a: <strong class="text-white">{{ p.username }}</strong></span>
                  </div>
                } @else {
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
            <div class="md:col-span-6">
              <div class="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3">
                <p class="text-xs font-bold uppercase tracking-wider text-[var(--color-text-secondary)] mb-3">
            ...
            <div class="md:col-span-6 space-y-3">
              <p class="text-xs font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">
              ...
              @if (rows.length > 0) {
                <div class="space-y-2 max-h-[360px] overflow-y-auto scrollbar-thin pr-1">
                  @for (row of rows; track row.slot) {
                    <div class="p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-2)] space-y-2">
                      <div class="flex items-center justify-between gap-2">
                        <span class="text-xs font-bold text-white">{{ row.itemName }}</span>
                        <span class="chip chip--neutral font-mono text-[10px] uppercase">{{ row.slot }}</span>
                      </div>
                      ...
                      <div class="grid gap-1.5">
                        @for (slot of row.slots; track slot.label + slot.index) {
                          @let chosen = selectedChoiceInSlot(slot);
                          @if (chosen) {
                            <div class="flex items-center gap-2.5 rounded-lg bg-[var(--color-surface)] px-2.5 py-1.5 border border-white/5">
                              <span class="font-mono text-xs font-bold px-1.5 py-0.5 rounded bg-white/10 text-white min-w-6 text-center">
                                {{ slot.label }}
                              </span>
                              <img
                                [src]="iconUrlForSpell(chosen.id)"
                              ...
                              />
                              <div class="min-w-0 flex-1">
                                <p class="text-xs font-semibold text-white truncate">{{ chosen.name }}</p>
                              </div>
                            </div>
                          }
                        }
                ...
                <p class="text-xs text-[var(--color-text-secondary)] p-4 rounded-lg bg-[var(--color-surface-2)] text-center">
        ...
        <div dialogFooter>
          <button type="button" class="btn btn--outline btn--sm" (click)="inspectDialogSeat.set(null)">
            Chiudi
          </button>
        </div>
      </app-dialog>
    ...
    @if (activeWeaponTooltip(); as tip) {
      ...
      >
        <div class="flex items-center gap-2.5 pb-2 mb-2 border-b border-white/10">
          @if (tip.icon) {
            <img [src]="tip.icon" [alt]="tip.name" class="h-8 w-8 object-contain rounded-md border border-white/10 bg-white/5 p-0.5" />
          }
          <div class="min-w-0 flex-1">
            <p class="text-xs font-bold text-white truncate">{{ tip.name }}</p>
            @if (tip.tier) {
              <span class="text-[10px] font-mono text-[var(--color-primary)] font-semibold">{{ tip.tier }}</span>
            }
          </div>
        </div>
        ...
        @if (tip.spells.length > 0) {
          <div class="space-y-1.5">
            <p class="text-[9px] uppercase font-bold tracking-wider text-[var(--color-text-tertiary)]">Abilità selezionate</p>
            @for (spell of tip.spells; track spell.key + spell.name) {
              <div class="flex items-center gap-2 rounded-md bg-white/5 px-2 py-1 border border-white/5">
                <span class="font-mono text-[10px] font-bold px-1.5 py-0.2 rounded bg-white/10 text-white min-w-5 text-center">
                  {{ spell.key }}
                </span>
                @if (spell.iconUrl) {
                  <img [src]="spell.iconUrl" [alt]="spell.name" class="h-5 w-5 rounded object-contain" />
                }
          ...
          <p class="text-xs text-[var(--color-text-secondary)] italic">Nessuna abilità per questa selezione</p>
        }
      </div>
    }
  `,
...
export class EventDetailPage {
  ...
  protected seatWeaponSpells(seat: EventRosterSeat): { key: string; name: string; iconUrl: string }[] {
  ...
  protected selectedChoiceInSlot(slot: AbilitySlotView) {
    ...
    return slot.choices.find((c) => c.id === slot.selected) ?? { id: slot.selected, name: slot.selected };
  }
