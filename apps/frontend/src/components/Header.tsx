...
export default function Header() {
  ...
  return (
    <>
      {/* Dynamic Styling to shift the sibling <main> content area */}
      <style dangerouslySetInnerHTML={{ __html: `
        /* Overrides to adapt existing page structures to the new dark theme design */
        div.min-h-screen {
          background-color: var(--color-workspace-bg) !important;
        ...
        /* Style existing light-themed cards to look premium and dark themed */
        div.rounded-3xl.bg-white, 
        div.rounded-3xl.bg-zinc-50,
        div.rounded-2xl.bg-zinc-50 {
      ...
      {/* Sidebar Container */}
      <aside className={`w-[260px] bg-sidebar flex-none border-r border-border-dark flex flex-col justify-between transition-all duration-300 md:translate-x-0 fixed left-0 top-0 bottom-0 z-30 ${
        sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
      }`}>
        <div className="flex flex-col overflow-y-auto flex-1">
          {/* Logo Section */}
          <div className="p-6 flex items-center gap-3 border-b border-border-dark h-[73px]">
      ...
      {/* Top Header Bar */}
      <header className="h-[73px] bg-header border-b border-border-dark flex items-center justify-between px-6 sticky top-0 z-20 md:left-[260px] left-0 right-0">
