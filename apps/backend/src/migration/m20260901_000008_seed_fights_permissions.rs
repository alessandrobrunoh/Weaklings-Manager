//! Seeds the `fights.manage` permission into `role_permissions` for Admin and Officer roles.

use sea_orm_migration::prelude::*;

/// Migration step to seed fight permissions.
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let fights_perms = ["fights.manage"];
        let seeds: &[(&str, &[&str])] = &[
            ("Admin", fights_perms.as_slice()),
            ("Officer", fights_perms.as_slice()),
        ];
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        // Remove the fights.manage permission from all roles.
        db.execute(
            backend.build(
                &Query::delete()
                    .from_table(RolePermissions::Table)
                    .and_where(Expr::col(RolePermissions::Permission).eq("fights.manage"))
                    .to_owned(),
            ),
        )
        .await?;
