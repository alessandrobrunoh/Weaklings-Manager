//! Seeds `splits.islands.manage` onto the Admin role.
        ...
        db.execute(
            backend.build(
                &Query::insert()
                    ...
                    .values_panic([admin_role_id.clone().into(), "splits.islands.manage".into()])
        ...
        db.execute(
            backend.build(
                &Query::delete()
                    ...
                    .and_where(Expr::col(RolePermissions::Permission).eq("splits.islands.manage"))
                    .to_owned(),
            ),
        )
        .await?;
