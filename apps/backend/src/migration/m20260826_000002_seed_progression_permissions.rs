//! Seeds the `progression.*`, `warns.*`, and `vod.submit` permissions into `role_permissions`.
//!
//! - `User` (every authenticated member) gets `progression.view` and `vod.submit`.
//! - `Moderator` additionally gets `progression.adjust`, `warns.view`, `warns.issue`.
//! - `Admin` gets all six (including `progression.settings.manage`).

use sea_orm_migration::prelude::*;

/// Migration step to seed progression permissions.
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let member_perms = ["progression.view", "vod.submit"];
        let officer_perms = [
            "progression.view",
            "vod.submit",
            "progression.adjust",
