...
function loadState(): PollerState {
  ...
  const defaultState: PollerState = { lastEventId: 0, lastBattleId: 0, pinged1hEvents: [] };
...
export class Poller {
  ...
  private async poll(): Promise<void> {
    await Promise.allSettled([this.checkNewEvents(), this.checkNewBattles(), this.checkUpcomingEvents()]);
  }
  ...
  private async checkUpcomingEvents(): Promise<void> {
    if (!config.EVENT_PING_ROLE_ID) return;
    ...
    try {
      ...
      for (const event of upcomingEvents) {
        ...
        if (thread) {
          await thread.send(`🚨 <@&${config.EVENT_PING_ROLE_ID}> L'evento **${event.title}** inizia tra meno di 1 ora! Preparatevi!`);
        }
